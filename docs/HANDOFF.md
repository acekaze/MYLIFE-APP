...
sessions/{CODE}/
  ...
  adminNames/{authId}/        # 현재 세션의 관리자 표시 이름
  state/
...
- 이름 수정: 헤더에서 본인 이름 수정. 기존 투자·투자 안 함·이벤트 손익의 표시 이름도 함께 변경
- 헤더 로고 + 이름·팀, 턴 진행률 바
- 순수익 히어로(큰 숫자), 상품 선택 그리드, 투자 폼, "투자 안 함"
- 만기 도래 시 주사위 굴리기(직접 선택 또는 랜덤 애니메이션+사운드)
- 게임 종료 정산 시 미만기 투자를 포함한 본인 미정산 투자 전체에 직접 주사위 입력. 미입력 건은 관리자 대리 정산 가능
- 진행 중/완료 투자 목록, "내 결과 공유하기"(개인 지표만, 순위 없음)
- 현재 턴 투자: 종목과 금액 수정 가능. 턴이 넘어가거나 정산 단계가 시작되면 수정 불가
...
- 이번 턴 투자: 강사가 참가자별 종목·금액 수정 가능. 턴이 넘어가거나 정산 단계가 시작되면 수정 불가
