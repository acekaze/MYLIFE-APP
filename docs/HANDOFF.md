...
### 상품 4종 (초기값)
| 상품 | 수익률 | 손실률 | 중도해약이율 | 성공 주사위 | 보존 | 실패 | 최소금액 |
|------|--------|--------|------|------|------|------|------|
| 채권형펀드 | 8% | 0% | 4% | 1,2,3,5,6 | 4 | - | 500 |
| 주식형펀드 | 16% | -20% | 8% | 1,2,3,6 | 4 | 5 | 500 |
| 고위험ETF | 40% | -50% | 20% | 1,2,3,4 | 5 | 6 | 500 |
| 선물/옵션 | 60% | -60% | 30% | 1,2,3 | 4 | 5,6 | 500 |
...
- 게임 종료(최종 턴) 시 미만기 투자는 주사위 굴려 **중도해약 정산**: 성공→중도해약이율 수익 / 실패→원래 손실률의 절반 / 보존→원금
- 월드이벤트: 관리자가 종목 선택 → 통합 대표 주사위 1회 → 해당 종목 전원 즉시 정산 (`settledBy: worldEvent`)
