(()=>{const q=new URLSearchParams(location.search),sid=q.get('session'),pid=localStorage.getItem('mylife_player_id');if(!sid||!pid)return;const db=firebase.database(),root=db.ref(`sessions/${sid}/integrated/${pid}`),state=db.ref(`sessions/${sid}/state`);IntegratedAssets.watch(sid);SettlementFlow.watch(sid,pid,true);let turn=1,step=0,open=false,cash=1000,time=8,score=0,achieved=[],hand=[],products=[['채권형펀드',4,0],['주식형펀드',8,-10],['고위험ETF',20,-30],['선물/옵션',30,-40]];const el=id=>document.getElementById(id),c=el('content'),save=x=>root.update({...x,cash,time,score,turn,achieved,hand,updatedAt:Date.now()});function frame(){el('status').textContent=`${localStorage.getItem('mylife_player_name')||'참가자'} · 현금 ${cash}만 · 시간 ${time}개 · 투자 중 ${window.principal||0}만 · 정산 손익 ${window.investmentProfit||0}만 · 만족도 ${score}점`;BucketUI.gallery(el('handFrame'),hand,'내 버킷 카드 '+hand.length+'장');BucketUI.album(el('achievedFrame'),achieved,turn,!!closingTurn||sessionPhase==='ended');}let furthest=0,turnActions={},busy=false,sessionPhase='investing',closingTurn=0,negotiations={},quarterReviews={},salaryLevel=6;
...
async function onceAction(key,change,next){
 ...
 try{const session=(await db.ref('sessions/'+sid).once('value')).val();const liveState=session?.state;const settlement=SettlementFlow.inspect(session||{},pid);if((liveState?.currentTurn||1)!==turn||!['investing','settling'].includes(liveState?.phase)||settlement.pending.length||settlement.results.length)throw new Error('턴 변경');await root.once('value');const result=await root.transaction(d=>{
...
function render(){if(busy&&el('salaryWheel'))return;el('negotiationPopup')?.remove();if(!closingTurn)el('bucketReview')?.remove();el('bucketChooser')?.remove();el('bucketDetail')?.remove();el('abilityPopup')?.remove();el('salaryReceivePopup')?.remove();el('personalEventPopup')?.remove();if(!personalEventDue(turn)&&step===0)step=1;if(personalEventDue(turn)&&!actions().event&&sessionPhase==='investing')step=0;if(!el('boardNav')){const nav=document.createElement('nav');nav.id='boardNav';nav.innerHTML='<button id="tabBoard">개인판</button> <button id="tabInvest">투자</button> <button id="tabAchieved">이룬 버킷</button> <button id="tabAssets">자산 현황</button>';c.before(nav);el('tabBoard').onclick=()=>c.scrollIntoView();el('tabInvest').onclick=()=>{if(furthest>=5||sessionPhase==='settling'||sessionPhase==='finalSettling')location.href='/player/?session='+encodeURIComponent(sid)+'&module=investment';else alert('개인 행동을 완료한 뒤 투자할 수 있습니다.');};el('tabAchieved').onclick=()=>el('achievedFrame')?.scrollIntoView();el('tabAssets').onclick=()=>el('status').scrollIntoView();}if(!el('handFrame')){for(const id of ['handFrame','achievedFrame']){const section=document.createElement('section');section.id=id;section.className='card';c.after(section);}}if(closingTurn){frame();
...
const revealed=open||actions().event;
popup.innerHTML='<section style="width:min(480px,100%);text-align:center;color:white"><p>Q'+turn+' · 개인 이벤트</p><button id="revealEvent" style="width:100%;min-height:340px;border:3px solid #e9c66c;background:'+(revealed?'#fff5df':'#1d315e')+';color:'+(revealed?'#17243d':'white')+';border-radius:24px;padding:32px;font-size:24px">'+(revealed?'예상 밖의 지출<br><span style="display:block;font-size:18px;margin-top:24px">현금 100만 원 차감<br>시간·만족도 변화 없음</span>':'개인 이벤트 카드<br><span style="font-size:16px">눌러서 공개하세요</span>')+'</button><p>'+(revealed?'화면을 서로 보여주며 이야기를 나누세요.':'')+'</p>'+(revealed?'<button id="completeEvent" style="width:100%;padding:18px">공유 완료 · 임금 수령으로</button>':'')+'</section>';
