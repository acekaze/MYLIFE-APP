...
function render(){el('bucketChooser')?.remove();el('bucketDetail')?.remove();el('abilityPopup')?.remove();el('salaryReceivePopup')?.remove();el('personalEventPopup')?.remove();if(!personalEventDue(turn)&&step===0)step=1;if(personalEventDue(turn)&&!actions().event&&sessionPhase==='investing')step=0;if(!el('boardNav')){const nav=document.createElement('nav');nav.id='boardNav';nav.innerHTML='<button id="tabBoard">개인판</button> <button id="tabInvest">투자</button> <button id="tabAchieved">이룬 버킷</button> <button id="tabAssets">자산 현황</button>';c.before(nav);el('tabBoard').onclick=()=>c.scrollIntoView();el('tabInvest').onclick=()=>{if(furthest>=5||sessionPhase==='settling'||sessionPhase==='finalSettling')location.href='/player/?session='+encodeURIComponent(sid)+'&module=investment';else alert('개인 행동을 완료한 뒤 투자할 수 있습니다.');};el('tabAchieved').onclick=()=>el('achievedFrame')?.scrollIntoView();el('tabAssets').onclick=()=>el('status').scrollIntoView();}if(!el('handFrame')){for(const id of ['handFrame','achievedFrame']){const section=document.createElement('section');section.id=id;section.className='card';c.after(section);}}if(closingTurn){frame();negotiationScreen();const reviewButton=document.createElement('button');reviewButton.textContent='이번 4턴 버킷 나누기';reviewButton.onclick=()=>{const group=BucketUI.groupAchievements(achieved,turn);BucketUI.review(group.current,group.currentPeriod);};c.prepend(reviewButton);return;}furthest=Math.max(furthest,step);for(let i=0;i<6;i++){const t=el('s'+i);t.classList.toggle('on',i===step);t.style.display=i===0&&!personalEventDue(turn)?'none':'';t.onclick=()=>{if(i<=furthest){step=i;render()}}}el('title').textContent=`Q${turn} · ${['개인 이벤트','임금 수령','버킷 채우기','업무능력 투자','버킷 처리','상품 투자'][step]}`;frame();if(step===0){
