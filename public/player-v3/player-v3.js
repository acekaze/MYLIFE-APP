(()=>{const q=new URLSearchParams(location.search),sid=q.get('session'),pid=localStorage.getItem('mylife_player_id');if(!sid||!pid)return;const db=firebase.database(),root=db.ref(`sessions/${sid}/integrated/${pid}`),state=db.ref(`sessions/${sid}/state`);IntegratedAssets.watch(sid);SettlementFlow.watch(sid,pid,true);TeamPlay.watchSalary(sid,pid);let turn=1,step=0,open=false,cash=1000,time=8,score=0,achieved=[],hand=[],products=[['채권형펀드',4,0],['주식형펀드',8,-10],['고위험ETF',20,-30],['선물/옵션',30,-40]];const el=id=>document.getElementById(id),c=el('content'),save=x=>root.update({...x,cash,time,score,turn,achieved,hand,updatedAt:Date.now()});function frame(){el('status').textContent=`${localStorage.getItem('mylife_player_name')||'참가자'} · 현금 ${cash}만 · 시간 ${time}개 · 투자 중 ${window.principal||0}만 · 정산 손익 ${window.investmentProfit||0}만 · 만족도 ${score}점`;BucketUI.gallery(el('handFrame'),hand,'내 버킷 카드 '+hand.length+'장');BucketUI.album(el('achievedFrame'),achieved,turn,!!closingTurn||sessionPhase==='ended');}let furthest=0,turnActions={},busy=false,sessionPhase='investing',closingTurn=0,negotiations={},quarterReviews={},salaryLevel=6;
