(()=>{const q=new URLSearchParams(location.search),sid=q.get('session'),pid=localStorage.getItem('mylife_player_id');if(!sid||!pid){document.getElementById('title').textContent='참가자 링크로 입장해 주세요';const link=document.createElement('a');link.href='/player/'+(sid?'?session='+encodeURIComponent(sid):'');link.textContent='참가자 입장';document.getElementById('content').append(link);return;}const db=firebase.database(),root=db.ref(`sessions/${sid}/integrated/${pid}`),state=db.ref(`sessions/${sid}/state`);IntegratedAssets.watch(sid);SettlementFlow.watch(sid,pid,true);TeamPlay.watchSalary(sid,pid);let turn=1,step=0,open=false,cash=1000,time=8,score=0,achieved=[],hand=[],products=[['채권형펀드',4,0],['주식형펀드',8,-10],['고위험ETF',20,-30],['선물/옵션',30,-40]];const el=id=>document.getElementById(id),c=el('content'),save=x=>root.update({...x,cash,time,score,turn,achieved,hand,updatedAt:Date.now()});function frame(){BoardShell.paint({sid,turn,phase:sessionPhase,closing:closingTurn,furthest,salaryLevel,salaryRows,cash,time,score,achieved,ability:window.abilityCount||0,principal:window.principal||0,profit:window.investmentProfit||0,name:localStorage.getItem('mylife_player_name')||'참가자',teamName,discardedTime,maxTurns});BucketUI.gallery(el('handFrame'),hand,'내 버킷 '+hand.length+'/5');BoardShell.decorateHand(hand.length);BucketUI.album(el('achievedFrame'),achieved,turn,!!closingTurn||sessionPhase==='ended');}let furthest=0,turnActions={},busy=false,sessionPhase='investing',closingTurn=0,negotiations={},quarterReviews={},salaryLevel=6,teamName='',discardedTime=0,maxTurns=20;
